...
export default function DishTypes({ dishTypes }:  DishTypesProps) {
      ...
      <section className="overflow-hidden bg-sidebar border border-border flex flex-col rounded-md">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <div className="flex items-center gap-2">
                <h2 className="font-bold text-lg">Dish Types</h2>
        ...
        <TabsContent value="simplified" asChild>
          <div className="flex flex-wrap gap-2 p-3">
            {
              dishTypes.map((dt) => (
                <div key={dt.id} className="flex-1 font-semibold min-w-[150px] text-center text-sm bg-mealicious-primary text-white py-2 px-3 rounded-sm">
                  {dt.name}
                </div>
              ))
            }
          ...
          <div className="flex flex-col sm:grid sm:grid-cols-2 gap-2 p-3">
            {
              dishTypes.map((dt) => (
                ...
                >
                  <h2 className="font-bold text-lg">{dt.name}</h2>
                  <Separator />
                  <p>{dt.description}</p>
                </div>
              ))
