...
import { EditMealFormSchema, MAX_MEAL_TAGS_LENGTH } from "@/lib/zod/meal";
import { Info } from "lucide-react";
import { useCallback, useState } from "react";
import { useFormState, useWatch } from "react-hook-form";
import { useEditMealFormContext } from "@/components/meals/edit/edit-meal-form";

const TagsSchema = EditMealFormSchema.shape.tags;

export default function MealTags() {
  const [tag, setTag] = useState("");
  const [touched, setTouched] = useState(false);
  
  const { control, setValue } = useEditMealFormContext();
