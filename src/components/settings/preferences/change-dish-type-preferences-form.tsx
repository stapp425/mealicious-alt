...
export default function ChangeDishTypePreferencesForm({ dishTypePreferences }: ChangeDishTypePreferencesFormProps) {
  ...
  const { execute, isExecuting } = useAction(updateDishTypePreferences, {
  ...
  return (
    <form onSubmit={handleSubmit(execute)} className="grid overflow-hidden">
      ...
      <Collapsible open={open} onOpenChange={setOpen} className="grid border border-border mt-4 rounded-md">
        <div className="grid">
          {
            fields.slice(0, MAX_DISH_TYPE_DISPLAY_LIMIT).map((d, index) => (
              <DishTypePreference 
                key={d.id}
                index={index}
                control={control}
        ...
        <CollapsibleContent className="grid">
          {
            fields.slice(MAX_DISH_TYPE_DISPLAY_LIMIT).map((d, index) => (
              <DishTypePreference 
                key={d.id}
...
const DishTypePreference = memo(({
  index,
  control,
  setPreferenceValue
}: {
  index: number;
  control: Control<ChangeDishTypePreferencesForm>;
  setPreferenceValue: UseFormSetValue<ChangeDishTypePreferencesForm>;
}) => {
